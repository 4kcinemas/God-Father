    ...
    if 2 < len(message.text) < 100:    
        ...
        if files:
            for file in files:
                ...
                filename = f"[🎬{get_size(file.file_size)}] 📂 [SS] {file.file_name}"
                btn.append(
...
async def group(client, message):
    ...
    if 2 < len(message.text) < 50:    
        ...
        if files:
            for file in files:
                file_id = file.file_id
                filename = f"🔗 [{get_size(file.file_size)}] 🎞️[𝗦𝗦] {file.file_name}"
                btn.append(
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if (clicked == typed):
        ...
        if query.data.startswith("next"):
            ...
            buttons = [
                [
                    InlineKeyboardButton('❕ Movie Group ❕', url=f'https://t.me/netflixmovies_sslinkz')
                ]
                ]
            ...
            for files in filedetails:
                ...
                buttons = [
                    [
                        InlineKeyboardButton('⭕ Join Our Channel ⭕', url=f'https://t.me/ss_linkz')
                    ]
                    ]
